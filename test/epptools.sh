...
ps: 此脚本用于为360epp服务器提供工具,方便进行排查

::* v1.3.1_20230424_beta
...
功能:
	1.自动根据不同系统,不同系统平台,不同软件版本,来安装和升级AGENT、杀毒产品
	2.可以手动选择需要安装和卸载的产品
	3.读取eset产品安装时,写入系统事件日志,方便出现安装错误时,定位问题
	4.读取系统状态用于判断问题
	5.可以根据需求自定义命令行参数，达到脚本自动运行安装的目的,(通过 DEFAULT_ARGS 参数实现)
	7.默认不加参数打开脚本会出现一个选择界面（此界面的操作会有一定的交互能力）
	8.支持命令行参数,以便远程批量推送时静默安装
...
使用方法：
	...
	4.
		agentVersion=10
		productVersion_efs=9.1
		productVersion_esets=4.5
		以上三个参数标识了最新的版本,一般版本号和安装文件的版本保持一致.
		当计算机已经存在一个杀毒软件,如果低于以上版本则会自动升级,如果高于则跳过安装,如果计算机没有安装过杀毒软件则预设版本号为0
	5. 这个参数 agentConfPath 指定的文件,来自于gpo配置文件,安装的时候可以不用手动填写证书、密码等之类的东西，可以通过eset控制台生成
	6.脚本的运行需要root 权限
	7.安装的时候脚本会自动检测需要下载的对应版本,不会多下载的,所以如果没有 内核低于 3.10 的系统,完全可以不填写 esets 的文件下载地址;这样并不会影响使用
	8.想到了再写
...
Usage: $0 [options]
  ...
  -u,  --gui             Show information As Gui
		...
		Example: $0 -g -p --gui
			...
			Code by kermit.yao @ centos 7.9, 2022-03-2, kermit.yao@qq.com
        ...
        then
            ...
            then
                ...
                then
                    ...
                    printLog $LINENO INFO agentDowanload "下载 Agent 安装文件:[${agentPath}]"
                    ...
                    then
                        ...
                        agentDownStatus=0
                        printLog $LINENO INFO installAgent "下载 Agent 配置文件:[${agentConfPath}]"
                ...
                then
                    ...
                    then
                        ...
                        then
                            ...
                            printLog $LINENO ERROR installAgent "读取配置文件失败:[${localAgentConfPath}]"
                        fi
                    fi
                fi
            else
