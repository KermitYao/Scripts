...
for /f "delims=" %%a in ('cscript //nologo %windir%\system32\slmgr.vbs -skms !kmsValue!') do echo 设置kms信息:%%a

for %%a in (!keyValue!) do (
	echo %%a
	for /f "delims=" %%b in ('cscript //nologo %windir%\system32\slmgr.vbs -ipk %%a^&^&set keyActivate=True') do (
		echo keyInstall:%%b
		if "!keyACtivate!"=="True" (
			goto :keyIsOk
		)
...
goto :eof
:keyIsOk
for /f "delims=" %%a in ('cscript //nologo %windir%\system32\slmgr.vbs -ato ^&^& set activateStatus=True') do (
	echo 执行激活:%%a
)

if "!activateStatus!"=="True" (
	set returnValue=!activateStatus!
)
